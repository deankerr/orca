...
export const commit = defineMutationSpec({
  ...
  handler: async (ctx, args) => {
    ...
    const viewId = await (async () => {
      if (state === null) {
        return ctx.db.insert('catalog_providers_views', materializedCurrent)
      }
    ...
    const snapshotId =
      state !== null && isIdentical
        ? state.snapshotId
        : await ctx.db.insert('catalog_providers_snapshots', args.next.content)

    await ctx.db.insert('catalog_providers_state', {

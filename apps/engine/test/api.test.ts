...
// *
// * Nothing here mocks the archive: `Archive.make` takes a bucket client, so the real store, the
...
import * as Api from '../src/api/http.ts'
import * as Archive from '../src/archive/store.ts'
...
const seeded = Effect.gen(function* seeded() {
  const archive = Archive.make(fakeBucket(new Map<string, Stored>()))
  const batch = batchIdAt(yield* DateTime.now)

  yield* archive.putCatalog({ batch, body: JSON.stringify({ data: [] }) })
...
const withArchive = async () => await Effect.runPromise(seeded)
...
describe('navigating the archive', () => {
  test('lists crawls, then one crawl in detail', async () => {
    const { batch, call } = await withArchive()

...
cron / POST /capture
  ...
  → load bodies → Convex current-view sink
```
...
| `src/resources/`            | Alchemy resources (R2, D1, queues)                             |
...
Product decode lives in `@orca/entities` or a product package. Adding a sink means returning the Sink
shape and appending it to `worker.ts`; the bus isolates plugin failures. Public API V2 is no longer an
engine sink and is independently deployed from `packages/public-api-v2`.
